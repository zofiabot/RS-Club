...
async def cmd_start_rs_queue(ctx: discord.ext.commands.Context,
	...
	for level in levels:
		if int(level) in Rs.star_range:
			if len(Rs.get_qm(int(level)).queue) > 1:
				...
				await bot.get_channel(ctx.message.channel.id).send(
				    f"{ctx.author.mention} you can only start queue early if you are not alone"
				)
				break
			counter += 1
		if counter == 1:
			await bot.get_channel(ctx.message.channel.id).send(
			    f"{ctx.author.mention} you can only start one star with this command"
			)
			break
	...
	try:
		...
		if message.author == intern_user and message.content == '!pig':
			...
			await message.channel.send(
			    f'{now.strftime("%H:%M")} Latency: {round(bot.latency, 3)}s.')
			await message.delete()
...
async def on_ready():
	...
	if not Rs.task_invite_ranking.is_running():
...
async def on_member_join(member):
	if member.id in params.SPECIAL_WELCOME_MEMBERS:
		...
		message = "!\u2800 ".join(
		    random.sample(params.TEXT_WELCOME_MESSAGES, n))
